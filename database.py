...
def modificar_so(activo_id, quitar, agregar, fecha_modificacion):
    try:
        # Realiza la conexión a la base de datos
        conn = mysql.connector.connect(**db_config)
        cursor = conn.cursor()
        ...
        if quitar:
            for elemento in quitar:
                query = f"UPDATE DISPO_SO SET OPERANTE = 0 WHERE ACTIVO_ID = {activo_id} AND SISTEMA_OPERATIVO_ID = {elemento} AND OPERANTE = 1 LIMIT 1"
                cursor.execute(query)
        
        if agregar:
            for elemento in agregar:
                insert_nuevo = "INSERT INTO DISPO_SO(ACTIVO_ID, SISTEMA_OPERATIVO_ID, FECHA_COLOC, OPERANTE) VALUES (%s, %s, %s, %s)"
...
def modificar_puerto(activo_id, quitar, agregar, fecha_modificacion):
    try:
        # Realiza la conexión a la base de datos
        conn = mysql.connector.connect(**db_config)
        cursor = conn.cursor()
        ...
        if quitar:
            for elemento in quitar:
                query = f"UPDATE DISPO_PUERTO SET OPERANTE = 0 WHERE ACTIVO_ID = {activo_id} AND PUERTO_ID = {elemento} AND OPERANTE = 1 LIMIT 1"
                cursor.execute(query)
        
        if agregar:
            for elemento in agregar:
                insert_nuevo = "INSERT INTO DISPO_PUERTO(PUERTO_ID, ACTIVO_ID, FECHA_COLOC, OPERANTE) VALUES (%s, %s, %s, %s)"
                cursor.execute(insert_nuevo, (elemento, activo_id, fecha_modificacion, 1))
